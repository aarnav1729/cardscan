      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in duration-200">
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 overflow-y-auto">
          <div className="flex justify-between items-start mb-8">
            <div>
              <h2 className="text-3xl font-bold text-slate-900 mb-1">{card.name || "Empty Name"}</h2>
              <p className="text-lg text-indigo-600 font-medium">{card.jobTitle || "No Title Specified"}</p>
            </div>
            <button 
              onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <DetailItem 
              ...
              value={card.website} 
              isLink
              href={card.website && !card.website.startsWith('http') ? `https://${card.website}` : card.website}
            />
            <DetailItem 
...
const DetailItem: React.FC<DetailItemProps> = ({ icon, label, value, isLink, href }) => {
